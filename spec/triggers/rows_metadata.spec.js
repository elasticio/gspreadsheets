describe('List available google spreadsheets', () => {
  const nock = require('nock'); let cfg; let cb; let
    that;
  const verify = require('../../lib/triggers/rows');
  ...
  it('Should provide message if no spreadsheet URL is set', () => {
    delete cfg.spreadsheetURL;

    verify.getMetaModel.call(that, cfg, cb);

    waitsFor(() => cb.callCount);
  ...
  it('should react on worksheet load failure', () => {
    // Refresh token
    ...
    // Load worksheet
    const scope3 = nock('https://spreadsheets.google.com')
  ...
  it('should load successfully', () => {
    ...
    runs(() => {
      ...
      const metadata = cb.calls[0].args[1];
