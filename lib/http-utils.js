...
exports.getJSON = function getJSON(params, cb) {
    ...
    that.logger.info(`Sending ${method} request`);
    ...
    request[method.toLowerCase()]({
        ...
        if (err) {
            that.logger.error('Failed to fetch JSON');
            return cb(err);
        }
        if (resp.statusCode === expectedStatus) {
            ...
            try {
                ...
                that.logger.error('Failed to parse JSON');
                cb(parseError);
            }
            if (result) {
                try {
                    that.logger.info(`Have got ${expectedStatus} response from ${method}`);
                    cb(null, result);
                } catch (e) {
                    that.logger.error('Exception happened when passing data down the chain');
                }
            } else {
