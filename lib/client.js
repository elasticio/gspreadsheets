...
const contextEmitWarner = {
  emit: () => { console.log("Can't emit new keys: no context found!") }
};
...
class GoogleOauth2Client {
  constructor(configuration, context) {
    ...
    this.context = context || contextEmitWarner;
    this.logger = (context && context.logger) ? context.logger : consoleLogger;
  ...
  buildClient() {
    ...
    this.client.on('tokens', (tokens) => {
      this.logger.debug('Token is outdated. Updating token...');
      this.logger.trace('New Oauth block: %j', tokens);
      ...
      this.logger.debug('New Token was emitted successfully...');
    });
  }
