...
const contextEmitWarner = {
  emit: () => { console.error("Can't emit new keys: no context found!") }
};
...
class GoogleOauth2Client {
  ...
  buildClient() {
    ...
    this.client.on('tokens', (tokens) => {
      debug('Token is outdated. Updating token...');
      debug('New Oauth block: %j', tokens);
      ...
      debug('New Token was emitted successfully...');
    });
  }
