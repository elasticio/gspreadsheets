...
class GoogleOauth2Client {
  ...
  async writeColumnToSpreadsheet(opts) {
    ...
    this.logger.debug('Spreadsheet column written successfully');
    return result;
  }
  ...
  async updateSpreadsheetColumn(opts) {
    this.logger.debug('Trying to update a column to spreadsheet...');
    const { spreadsheetId } = this.configuration;
    const { range, values } = opts;
    const sheets = google.sheets({ version: sheetsVersion, auth: this.client });
    const result = await sheets.spreadsheets.values.update({
    ...
    this.logger.debug('Spreadsheet column updated successfully');
    return result;
  }
