...
class GoogleOauth2Client {
  ...
  async buildClient() {
    ...
    const redirectUri = `https://${process.env.TENANT_DOMAIN || 'app.elastic.io'}/callback/oauth2`;
    this.client = new google.auth.OAuth2(clientId, clientSecret, redirectUri);
    this.client.setCredentials(token);
  }

  async runFunction(gsFunction, opts) {
    if (!this.client) {
    ...
    let result;
    for (let iteration = 1; iteration <= retries; iteration++) {
      ...
      try {
        ...
        this.logger.error('Error occurred during spreadsheet function: %j', e);
        if (e.code && ['401', '403'].includes(e.code)) {
          ...
          if (currentToken.access_token !== token.access_token) {
            ...
            try {
              ...
              token = await utils.refreshToken(this, this.configuration.secretId);
  ...
  async getDrive() {
    ...
    try {
      return await retry(async () => {
        try {
          const driveClient = google.drive({ version: 'v3', auth: this.client });
    ...
    try {
      return await retry(async () => {
        ...
        try {
          ...
          const res = await sheets.spreadsheets.values.batchGet(requestParams);
