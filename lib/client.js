...
class GoogleOauth2Client {
  ...
  /**
   * @returns object with list of "worksheetId" and "worksheet name" pairs
   * Example: { 'worksheetId1': 'worksheet name 1', 'worksheetId2': 'worksheet name 2', }
   */
  async listOfWorksheets(spreadsheetId) {
    ...
    data.sheets.forEach((item) => {
      result[item.properties.sheetId] = item.properties.title;
    });
    this.logger.debug('Worksheets listed successfully');
    console.log(88, JSON.stringify(result));
    return result;
  }

  async getWorksheetNameById(worksheetId) {
    const { spreadsheetId } = this.configuration;
    const idToName = await this.listOfWorksheets(spreadsheetId);
    if (idToName[worksheetId]) return idToName[worksheetId];
    throw new Error(`Worksheet with id "${worksheetId}" is not found`);
  }
  ...
  async getSpreadsheet(override) {
    this.logger.debug('Trying to get spreadsheet...');
    const { spreadsheetId, worksheetId } = this.configuration;
    const worksheetName = await this.getWorksheetNameById(worksheetId);
    console.log(111, worksheetId, worksheetName);
    const sheets = google.sheets({ version: sheetsVersion, auth: this.client });
  ...
  async writeToSpreadsheet(opts) {
    this.logger.debug('Trying to write a row to spreadsheet...');
    const { spreadsheetId, worksheetId, dimension = 'ROWS' } = this.configuration;
    const worksheetName = await this.getWorksheetNameById(worksheetId);
    let { range } = opts;
    const { values } = opts;
    if (dimension === 'ROWS') {
  ...
  async updateSpreadsheetRow(opts) {
    this.logger.debug('Trying to update a row in spreadsheet...');
    const { spreadsheetId, dimension } = this.configuration;
    ...
    this.logger.debug('Spreadsheet row updated successfully');
    return result;
  }
