...
function fetchWorkbook(cfg) {
  ...
  function applyNewCfg(updatedCfg) {
    ...
    that.logger.info('Refreshed token, received new keys');
  }

  function fetchSpreadsheet() {
    that.logger.info('Fetching spreadsheet...');
  ...
  function findAndFetchWorksheet(body) {
    ...
    that.logger.info('Fetching worksheet url...');
  ...
  oAuthUtils.refreshTokenPromise.call(that, 'gspreadsheets', cfg)
    .then(applyNewCfg)
    .then(fetchSpreadsheet)
    .then(findAndFetchWorksheet)
    .then(complete)
    .fail((err) => {
      that.logger.error('Fetch Workbook Failed');
      defered.reject(err);
    })
    .done();
...
function listSpreadsheets(cfg, cb) {
  ...
  function onError(err) {
    that.logger.error('Spreadsheets list failed');
    cb(err);
  }
