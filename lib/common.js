...
function fetchWorkbook(cfg) {
    ...
    function fetchSpreadsheet(cfg) {
        console.log('Fetching spreadsheet url=%s', spreadsheetURL);
        return http.read(appendParams(spreadsheetURL, cfg));
    }
    ...
    function findAndFetchWorksheet(body) {
        ...
        return http.read(appendParams(worksheetUrl, cfg));
    }

    function complete(body) {
        var response = JSON.parse(body.toString('utf-8'));
        var feed = response.feed;
        defered.fulfill(feed);
    }
...
function allSpreadsheets(cfg, cb) {
    ...
    oAuthUtils.refreshTokenPromise('gspreadsheets', cfg)
        .then(function onTokenReceived(conf) {
        ...
        .then(function onSuccess(body) {
            ...
            var docs = response.feed.entry.map(function processEntry(entry) {
...
function extractCellValues(row, nsholder, callback) {
    _.keys(row).forEach(function extractRowCell(key) {
...
function getMetaModel(cfg, cb) {
    fetchWorkbook(cfg).then(function onSuccess(feed) {
