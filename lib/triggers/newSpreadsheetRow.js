...
const { transformToOutputStructure, columnToLetter } = require('../common');
...
async function processTrigger(msg, cfg, snapshot) {
  ...
  const drive = await googleOauth2Client.callFunction(googleOauth2Client.getDrive);
  ...
  const ranges = [];
  const worksheetName = await googleOauth2Client.callFunction(googleOauth2Client.getWorksheetNameById, cfg.worksheetId);
  if (includeHeader) {
    ...
    if (cfg.dimension === 'ROWS') {
      ranges.push(`${worksheetName}!A1:${columnToLetter(5000)}1`);
      ranges.push(`${worksheetName}!A${snap.lastEmittedLine + 1}:${columnToLetter(5000)}${snap.lastEmittedLine + 1000}`);
    } else if (cfg.dimension === 'COLUMNS') {
      ranges.push(`${worksheetName}!A1:A5000`);
      ranges.push(
        `${worksheetName}!${columnToLetter(snap.lastEmittedLine + 1)}1:${columnToLetter(snap.lastEmittedLine + 1000)}5000`,
      );
    }
    ...
    ranges.push(`${worksheetName}!${columnToLetter(snap.lastEmittedLine + 1)}1:${columnToLetter(
  ...
  const responseRange = await googleOauth2Client.callFunction(googleOauth2Client.batchGetRows, requestParams);
