  ...
  if (snap.modifiedTime < newModifiedTime) {
    ...
    if (includeHeader) {
      ...
      if (cfg.dimension === 'ROWS') {
        ...
        ranges.push(`${cfg.worksheetName}!${columnToLetter(snap.lastEmittedLine + 1)}1:${columnToLetter(snap.lastEmittedLine + 1000)}${letterToColumn('ZZZ')}`);
      }
    ...
    await googleOauth2Client.checkAndUpdateCredentials(googleOauth2Client.credentials);
