...
async function insertRowOrColumn(headerNames, dimension, body, oAuth2Client, worksheetId, spreadsheetValues) {
  ...
  return oAuth2Client.callFunction(oAuth2Client.writeToSpreadsheet, opts);
}
...
async function processAction(msg, cfg) {
  ...
  let result;
  if (dimension === 'ROWS') {
    const headersArray = spreadsheetValues[0];
    ...
    if (isCellDataPresent === 0) {
      ...
      result = await insertRowOrColumn(headerNames, dimension, body, oAuth2Client, cfg.worksheetId, spreadsheetValues);
    } else {
      this.logger.info('Data found by upsert criteria, going to update existing row');
      const range = `${cfg.worksheetId}!A${rowOrColumnPosition + 1}:${columnToLetter(headerNames.length)}${rowOrColumnPosition + 1}`;
      this.logger.debug('Calculated range: %s', range);
      const inputData = getUpdateInputData(spreadsheetValues[rowOrColumnPosition], headerNames, body);
      result = await oAuth2Client.callFunction(oAuth2Client.updateSpreadsheetRow, { range, values: inputData });
    }
  } else if (dimension === 'COLUMNS') {
    const headersArray = spreadsheetValues;
    const headerNames = getHeaderNames(headersArray, dimension, mode);
    const { isCellDataPresent, rowOrColumnPosition } = getExistingCell(dimension, mode, headerNames, body, upsertCriteria, spreadsheetValues);
    if (isCellDataPresent > 1) {
      throw new Error('More than one rows found');
    ...
    if (isCellDataPresent === 0) {
      ...
      const range = `${cfg.worksheetId}!${columnToLetter(rowOrColumnPosition + 1)}1:${columnToLetter(rowOrColumnPosition + 1)}${headerNames.length}`;
      this.logger.debug('Calculated range: %s', range);
      const existingData = spreadsheetValues.map(val => val[rowOrColumnPosition]);
      const inputData = getUpdateInputData(existingData, headerNames, body);
