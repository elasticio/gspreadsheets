...
async function updateRowOrColumn(worksheetId, dimension, rowOrColumnPosition, headerNames, spreadsheetValues, body, oAuth2Client) {
  let existingData;
  let range;
  if (dimension === 'ROWS') {
  ...
  return oAuth2Client.callFunction(oAuth2Client.updateSpreadsheetRow, opts);
}
...
async function processAction(msg, cfg) {
  ...
  if (isCellDataPresent === 0) {
    ...
    result = await updateRowOrColumn(cfg.worksheetId, dimension, rowOrColumnPosition, headerNames, spreadsheetValues, body, oAuth2Client);
  }
