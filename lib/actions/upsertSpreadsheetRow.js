...
function getInsertRowOrColumnOpts(headerNames, dimension, body, worksheetName, spreadsheetValues) {
  const inputData = getInsertInputData(headerNames, body);
  const opts = { values: inputData };
  if (dimension === COLUMNS) {
    opts.range = `${worksheetName}!${columnToLetter(spreadsheetValues[0].length + 1)}1:${columnToLetter(spreadsheetValues[0].length + 1)}${headerNames.length}`;
  }
  return opts;
}

function getUpdateRowOrColumnOpts(worksheetName, dimension, rowOrColumnPosition, headerNames, spreadsheetValues, body) {
  let existingData;
  let range;
  if (dimension === ROWS) {
    range = `${worksheetName}!A${rowOrColumnPosition + 1}:${columnToLetter(headerNames.length)}${rowOrColumnPosition + 1}`;
    existingData = spreadsheetValues[rowOrColumnPosition];
  }
  if (dimension === COLUMNS) {
    range = `${worksheetName}!${columnToLetter(rowOrColumnPosition + 1)}1:${columnToLetter(rowOrColumnPosition + 1)}${headerNames.length}`;
...
async function processAction(msg, cfg) {
  ...
  const worksheetName = await oAuth2Client.callFunction(oAuth2Client.getWorksheetNameById, worksheetId);
  let result;
  let headersArray;
  if (dimension === ROWS) {
  ...
  if (isCellDataPresent === 0) {
    ...
    const insertOpts = getInsertRowOrColumnOpts(headerNames, dimension, body, worksheetName, spreadsheetValues);
    ...
    const updateOpts = getUpdateRowOrColumnOpts(worksheetName, dimension, rowOrColumnPosition, headerNames, spreadsheetValues, body);
