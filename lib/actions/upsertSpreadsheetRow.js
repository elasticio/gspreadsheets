...
async function processAction(msg, cfg) {
  ...
  if (dimension === 'ROWS') {
    ...
    if (isCellDataPresent === 0) {
      ...
      const range = `${cfg.worksheetId}!A${rowPosition + 1}:${columnToLetter(headerNames.length)}${rowPosition + 1}`;
      this.logger.debug('Calculated range: %s', range);
      const inputData = getUpdateInputData(spreadsheetValues[rowPosition], headerNames, body);
    ...
    if (isCellDataPresent === 0) {
      ...
      const range = `${cfg.worksheetId}!${columnToLetter(columnPosition + 1)}1:${columnToLetter(columnPosition + 1)}${headerNames.length}`;
      this.logger.debug('Calculated range: %s', range);
