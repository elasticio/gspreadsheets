...
function getHeaderNames(headersArray, dimension) {
...
async function processAction(msg, cfg) {
  ...
  if (dimension === 'ROWS') {
    ...
    if (mode === HEADER) {
      columnNames = getHeaderNames(headersArray, dimension);
      startRowPosition = 1;
    }
    if (mode === ARRAY) {
    ...
    if (mode === HEADER) {
      rowNames = getHeaderNames(headersArray, dimension);
      startColumnPosition = 1;
    }
    if (mode === ARRAY) {
      for (let j = 1; j < headersArray.length + 1; j++) {
