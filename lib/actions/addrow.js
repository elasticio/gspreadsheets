...
function getMetaModel(cfg, cb) {
  console.log('Request to add row meta model arrived');
  ...
  function buildMetaModel(workbook) {
    ...
    const firstRow = _.first(workbook.entry);
    const metadata = {
      out: {
        type: 'object',
        properties: {
    ...
    common.extractCellValues(firstRow, workbook, (cellName) => {
...
function executeAction(msg, cfg) {
  ...
  console.log('Request to add row arrived');
  ...
  function addNewRow(workbook, newCfg) {
    ...
    const postLink = _.find(workbook.link, isPostLink);
    ...
    const fields = [];
    const firstRow = _.first(workbook.entry);
    common.extractCellValues(firstRow, workbook, (field) => {
      fields.push(field);
    });

    const payload = preparePayload(msg.body || {}, fields);
