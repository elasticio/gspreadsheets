...
function getMetaModel(cfg, cb) {
    ...
    const that = this;
    this.logger.info('Request to add row meta model arrived');
    ...
    function onError(err) {
        that.logger.error(err.stack || err);
        cb(err);
    }
}
...
function executeAction(msg, cfg) {
    ...
    that.logger.info('Request to add row arrived');
    ...
    function addNewRow(workbook, newCfg) {
        ...
        that.logger.info('Found URL that shall accept new lines for the worksheet url=%s', postURL);
        ...
        that.logger.info('Will POST payload to URL payload=multiline\n%s', payload);
        return http.read({
