...
const expectedPayload = '<entry xmlns="http://www.w3.org/2005/Atom" xmlns:gsx="http://schemas.google.com/spreadsheets/2006/extended">\n'
    + '\t<gsx:sku>foo</gsx:sku>\n'
    + '\t<gsx:description>&lt;xml&gt;injection&lt;/xml&gt;&lt;?DTD?&gt;</gsx:description>\n'
    + '\t<gsx:price>22</gsx:price>\n'
    + '</entry>';

describe('Adding row action', () => {
  const nock = require('nock'); let cfg; let
    self;
  const verify = require('../../lib/actions/addrow.js');
  ...
  it('should load successfully first time', () => {
    // Refresh token
    ...
    waitsFor(() => self.emit.callCount >= 3);
    ...
    runs(() => {
      ...
      expect(self.emit.calls[1].args[1].body).toEqual({
        sku: 'foo', bar: 'hasi', price: 22, description: '<xml>injection</xml><?DTD?>',
      });
    });
  });
});
